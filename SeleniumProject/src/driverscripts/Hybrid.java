...
public class Hybrid {
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		String res = null;
		String path = "D:\\ManishRecordings\\Workspace\\SeleniumProject\\src\\testdata\\Hybrid.xlsx";
		String outPath = "D:\\ManishRecordings\\Workspace\\SeleniumProject\\src\\results\\HybridRes.xlsx";
		...
		XSSFSheet EmpSht = wb.getSheet("EmpReg");
		
		int TcRc = TcSht.getLastRowNum();
		int TsRc = TsSht.getLastRowNum();
		int EmpRc = EmpSht.getLastRowNum();
					...
					if (TcId.equalsIgnoreCase(TsTcId)) {
						...
						case "Empreg":
							for (int k = 1; k <= EmpRc; k++) {
								String f = EmpSht.getRow(k).getCell(0).getStringCellValue();
								String l = EmpSht.getRow(k).getCell(1).getStringCellValue();
								String eid = EmpSht.getRow(k).getCell(2).getStringCellValue();
								res = om.orgEmpReg(f, l, eid);
								EmpSht.getRow(k).createCell(3).setCellValue(res);
							}
							break;
						case "Usereg":
